...
class OpenVPNUI:
    ...
    def create_tray_icon(self):
        image = Image.new("RGB", (64, 64), "#4cafa5")
        draw = ImageDraw.Draw(image)
        draw.rectangle((16, 16, 48, 48), fill="black")

        menu = pystray.Menu(
            pystray.MenuItem("Afficher", self.restore_window, default=True),
        ...
        self.tray_icon = pystray.Icon(
            ...
            "OpenVPN UI",
            menu,
            on_activate=self.restore_window  # clic gauche fiable
        )
    ...
    def create_ui(self):
        ...
        ctk.CTkButton(button_frame, text="Search", command=self.search_file, **button_kwargs).grid(row=0, column=0,
                                                                                                   padx=10, pady=5)
        ctk.CTkButton(button_frame, text="VPN disconnect", command=disconnect_all, **button_kwargs).grid(row=1,
                                                                                                         column=0,
                                                                                                         padx=10,
                                                                                                         pady=5)
        ctk.CTkButton(button_frame, text="Setting", command=self.open_settings, **button_kwargs).grid(row=2, column=0,
                                                                                                      padx=10, pady=5)
        ctk.CTkButton(button_frame, text="History", command=self.show_history, **button_kwargs).grid(row=3, column=0,
                                                                                                     padx=10, pady=5)
    ...
    def show_already_connected_popup(self, file_name):
        ...
        ctk.CTkButton(button_frame, text="Disconnect and continue", command=disconnect_and_continue, fg_color="#4cafa5",
                      hover_color="#3e8e87").pack(side="left", padx=10)
        ctk.CTkButton(button_frame, text="Abort", command=abort, fg_color="#d50000", hover_color="#b71c1c").pack(
            side="left", padx=10)
    ...
    def open_settings(self):
        ...
        current_dir_label = ctk.CTkLabel(settings_window, text=f"Current directory : {self.config['search_directory']}",
                                         font=("Helvetica", 12))
        ...
        ctk.CTkButton(settings_window, text="Set search directory", command=set_directory, **button_kwargs).pack(
            pady=20)
